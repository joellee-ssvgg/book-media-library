    ...
    return (<div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
      <form action={exportAction} className="grid content-start gap-5 border border-[#d8d2c4] bg-[#fffdf8] p-5">
        ...
        <div className="flex flex-wrap items-start gap-3">
          ...
          {exportState.message ? (<p className="pt-2 text-sm text-[#3f4945]">{exportState.message}</p>) : null}
        </div>

        {exportState.status === "exported" ? (<div className="grid gap-4 border border-[#b9d2c9] bg-[#f3faf7] p-4">
            ...
            {zipHref ? (<a className="inline-flex h-10 w-fit items-center border border-[#315f53] bg-[#315f53] px-4 text-sm font-medium text-white" download="mspf-export.zip" href={zipHref}>
                下载 MSPF ZIP
              </a>) : null}
          </div>) : null}
      </form>
      ...
      <form action={deletionAction} className="grid content-start gap-5 border border-[#d8d2c4] bg-[#fffdf8] p-5">
        ...
        <div className="grid gap-2">
          ...
          <select className="h-10 border border-[#c9c2b3] bg-white px-3 text-sm outline-none focus:border-[#315f53]" defaultValue="export_then_delete" id="deletionChannel" name="deletionChannel">
        ...
        <div className="grid gap-2">
          ...
          <input className="h-10 border border-[#c9c2b3] bg-white px-3 font-mono text-xs outline-none focus:border-[#315f53]" id="exportJobId" key={exportState.jobId ?? "empty-export-job"} name="exportJobId" placeholder="00000000-0000-0000-0000-000000000000" defaultValue={exportState.jobId ?? ""}/>
        ...
        <div className="grid gap-2">
          ...
          <input className="h-10 border border-[#c9c2b3] bg-white px-3 font-mono text-sm outline-none focus:border-[#315f53]" id="confirmation" name="confirmation" placeholder="DELETE"/>
        ...
        <div className="flex flex-wrap items-start gap-3">
          ...
          {deletionState.message ? (<p className="pt-2 text-sm text-[#3f4945]">{deletionState.message}</p>) : null}
        </div>

        {deletionState.status === "soft_deleted" || deletionState.status === "cooling_off" ? (<dl className="grid gap-2 border border-[#ead6ab] bg-[#fff8e8] p-4 font-mono text-xs">
            <div className="grid gap-1 md:grid-cols-[120px_1fr]">
              <dt>request_id</dt>
            ...
            {deletionState.softDeleteUntil ? (<div className="grid gap-1 md:grid-cols-[120px_1fr]">
                <dt>soft_until</dt>
                <dd>{deletionState.softDeleteUntil}</dd>
              </div>) : null}
            {deletionState.coolingUntil ? (<div className="grid gap-1 md:grid-cols-[120px_1fr]">
                <dt>cooling_until</dt>
