...
const topEntryFields = ["topEntryId1", "topEntryId2", "topEntryId3"] as const;
...
export function PublicProfileSettingsForm() {
  const [state, formAction, pending] = useActionState(
    updatePublicProfileSettingsAction,
    initialPublicProfileSettingsActionState,
  );
  ...
  return (
    <form action={formAction} className="grid gap-5 border border-[#d8d2c4] bg-[#fffdf8] p-5">
      ...
      <div className="grid gap-3 border-l-2 border-[#315f53] bg-[#eef4f1] px-4 py-3">
        ...
        {topEntryFields.map((fieldName, index) => (
