      ...
      <div className="grid gap-3 md:grid-cols-[96px_1fr]">
        ...
        <div className="grid content-start gap-3">
          <div>
            ...
            {candidate.description ? (<p className="mt-2 line-clamp-3 text-sm leading-6 text-[#5f665f]">
          ...
          <div className="grid gap-2 sm:grid-cols-[180px_140px_auto] sm:items-end">
            <label className="grid gap-1 text-sm font-medium">
              初始状态
              <select className="h-10 border border-[#c9c2b3] bg-white px-3 text-sm outline-none focus:border-[#315f53]" defaultValue="want_to_watch" name="status">
            ...
            <label className="grid gap-1 text-sm font-medium">
              评分
              <select className="h-10 border border-[#c9c2b3] bg-white px-3 text-sm outline-none focus:border-[#315f53]" defaultValue="" name="ratingX10">
                {ratingOptions.map((option) => (<option key={option.value} value={option.value}>
                    {option.label}
                  </option>))}
              </select>
            </label>
...
export function MovieAddClient({ query, candidates, notices }) {
    return (<div className="grid gap-6 lg:grid-cols-[320px_1fr]">
      <aside className="grid content-start gap-5 border border-[#d8d2c4] bg-[#fffdf8] p-5">
        <form className="grid gap-3" action="/add/movie" method="get">
          <label className="text-sm font-medium" htmlFor="query">
            电影搜索
          </label>
          <input className="h-10 border border-[#c9c2b3] bg-white px-3 text-sm outline-none focus:border-[#315f53]" defaultValue={query} id="query" name="q" placeholder="Inception"/>
