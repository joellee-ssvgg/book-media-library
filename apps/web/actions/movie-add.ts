...
function toFieldErrors(result: ReturnType<typeof addMovieEntryFormSchema.safeParse>) {
  return result.success ? undefined : result.error.flatten().fieldErrors;
}

export async function addMovieEntryAction(
  _previousState: AddMovieEntryActionState,
  formData: FormData,
): Promise<AddMovieEntryActionState> {
  ...
  const { data, error } = await supabase.client.rpc<AddMovieEntryRpcPayload>(
    "task15_create_movie_entry_from_provider",
    {
      input_provider: "tmdb",
