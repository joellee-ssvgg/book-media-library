...
import { completeOnboardingFormSchema, onboardingImportFormSchema, } from "@/schemas/onboarding";
function toOnboardingFieldErrors(result) {
    return result.success ? undefined : result.error.flatten().fieldErrors;
}
function toImportFieldErrors(result) {
    return result.success ? undefined : result.error.flatten().fieldErrors;
}
function favoriteFromForm(formData, index) {
...
async function readUploadPayload(formData, source) {
    ...
    return mspfToImportPayload(JSON.parse(text), file.name);
}
export async function completeOnboardingAction(_previousState, formData) {
    ...
    const { data, error } = await supabase.client.rpc("task14_complete_onboarding", {
...
export async function enqueueOnboardingImportAction(_previousState, formData) {
    ...
    const { data, error } = await supabase.client.rpc("task14_enqueue_import_job", {
