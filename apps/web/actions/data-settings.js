...
import { accountDeletionFormSchema, dataExportFormSchema, } from "@/schemas/data-settings";
function exportFieldErrors(result) {
    return result.success ? undefined : result.error.flatten().fieldErrors;
}
function deletionFieldErrors(result) {
    return result.success ? undefined : result.error.flatten().fieldErrors;
}
export async function generateMspfExportAction(_previousState, formData) {
    ...
    const { data: queued, error: enqueueError } = await supabase.client.rpc("task18_enqueue_mspf_export");
    if (enqueueError) {
        return {
            status: "db_error",
    ...
    const { data: exported, error: exportError } = await supabase.client.rpc("task18_process_own_export_job", {
...
export async function requestAccountDeletionAction(_previousState, formData) {
    ...
    const parsed = accountDeletionFormSchema.safeParse({
        ...
        exportJobId: typeof rawExportJobId === "string" && rawExportJobId.trim()
            ? rawExportJobId
            : undefined,
    ...
    const { data, error } = await supabase.client.rpc("task18_request_account_deletion", {
