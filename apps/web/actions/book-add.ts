...
} from "@/schemas/book-add";

type AddBookEntryRpcPayload = {
...
function toFieldErrors(result: ReturnType<typeof addBookEntryFormSchema.safeParse>) {
  return result.success ? undefined : result.error.flatten().fieldErrors;
}

export async function addBookEntryAction(
  _previousState: AddBookEntryActionState,
  formData: FormData,
): Promise<AddBookEntryActionState> {
  ...
  const { data, error } = await supabase.client.rpc<AddBookEntryRpcPayload>(
    "task14_create_book_entry_from_provider",
    {
