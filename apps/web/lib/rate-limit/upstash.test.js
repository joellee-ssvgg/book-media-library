...
describe("createUpstashRateLimitStore", () => {
    it("uses the Upstash REST pipeline with INCR and EXPIRE NX", async () => {
        const fetchImpl = vi.fn(async () => new Response(JSON.stringify([{ result: 1 }, { result: 1 }]), { status: 200 }));
        ...
        expect(fetchImpl).toHaveBeenCalledWith("https://example-upstash.io/pipeline", expect.objectContaining({
            body: JSON.stringify([
    ...
    it("throws when Upstash credentials are missing so the caller can fail-open", async () => {
        const store = createUpstashRateLimitStore({}, vi.fn());
        await expect(store.increment("rl:ip:missing:1", 60)).rejects.toThrow("Upstash Redis REST credentials are not configured");
    });
});
