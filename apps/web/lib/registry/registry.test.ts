...
import {
  getRegistry,
  isEnabledMediaTypeId,
  listEnabledMediaTypes,
  listEnabledRegistries,
} from "./index";

function expectValidStatusStateMachine(mediaType: "book" | "movie") {
  const registry = getRegistry(mediaType);
  const defaultStatus = registry.statusOptions.find(
    (status) => status.key === registry.defaultStatus,
  );
...
describe("Media Type Registry", () => {
  ...
  it("keeps existing business code away from media-type branching", () => {
    ...
    const filesToScan = [
      "actions/manual-work.ts",
      "components/domain/manual-work-form.tsx",
      "schemas/manual-work.ts",
    ];
    const mediaBranchPattern =
      /if\s*\([^)]*(?:mediaType|media_type)\s*={2,3}\s*["'](?:book|movie)["']/;
