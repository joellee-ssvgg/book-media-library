import type { ImportItem, ImportPayload } from "./types";
...
function optionalBool(value: string | undefined) {
  const normalized = optionalText(value);
  if (!normalized) {
    return undefined;
  }
...
export function genericCsvToImportPayload(
  csvText: string,
  ...
  const titleIndex = headers.findIndex((header) =>
    ["canonical_title", "title", "name"].includes(header),
  );
  ...
  const items: ImportItem[] = rows.slice(1).map((row, rowIndex) => {
    ...
    return {
      ...
      status:
        optionalText(record.get("status")) ?? (mediaType === "movie" ? "watched" : "want_to_read"),
      rating_x10: optionalInt(record.get("rating_x10")),
      favorite: optionalBool(record.get("favorite")),
      visibility_scope:
        (optionalText(record.get("visibility_scope")) as ImportItem["visibility_scope"]) ?? "private",
