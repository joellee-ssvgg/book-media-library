...
function googleBooksApiKey(environment: ProviderEnvironment) {
  return environment.googleBooksApiKey ?? process.env.GOOGLE_BOOKS_API_KEY;
}

function isbnExternalIds(industryIdentifiers: Record<string, unknown>[]) {
...
function normalizeGoogleBooksCandidate(raw: Record<string, unknown>): CanonicalCandidate {
...
export function createGoogleBooksProvider(environment: ProviderEnvironment = {}): MediaProvider {
  return {
    id: "googlebooks",
    mediaTypes: ["book"],
    rateLimit: {
