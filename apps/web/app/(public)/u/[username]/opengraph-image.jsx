...
export default async function PublicProfileOgImage({ params }) {
    ...
    if (data.status !== "active") {
        ...
        return new ImageResponse((<div style={{
          ...
          <div style={{ color: "#6d7f8c", display: "flex", fontSize: 30, marginTop: 28 }}>
            @{username}
          </div>
        </div>), size);
    }
    ...
    return new ImageResponse((<div style={{
        ...
        <div style={{ alignItems: "center", display: "flex" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", marginLeft: 28 }}>
            <div style={{ color: "#6d7f8c", display: "flex", fontSize: 30 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          <div style={{ color: "#2c6485", display: "flex", fontSize: 30, marginBottom: 18 }}>
            精选 Top-3
          </div>
          <div style={{ display: "flex", gap: 18 }}>
            {(topTitles.length ? topTitles : ["暂无公开精选"]).map((title, index) => (<div key={`${title}-${index}`} style={{
                ...
                <div style={{ color: "#6d7f8c", display: "flex", fontSize: 24 }}>#{index + 1}</div>
        ...
        <div style={{ color: "#4d5f6e", display: "flex", fontSize: 28 }}>
