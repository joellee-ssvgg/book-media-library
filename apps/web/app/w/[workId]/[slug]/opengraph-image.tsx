...
export default async function PublicWorkOgImage({ params }: PublicWorkOgProps) {
  const { workId, slug } = await params;
  const data = isUuid(workId) ? await getPublicWork(workId, slug) : { status: "not_found" as const };
      ...
      >
        ...
        >
          {data.edition?.cover_url ? (
            <img alt="" height="500" src={data.edition.cover_url} width="330" />
          ) : (
            <div style={{ color: "#315f53", display: "flex", fontSize: 44, fontWeight: 700 }}>
              书影计划
            </div>
          )}
        </div>
        ...
        >
          <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            {data.work.original_title ? (
              <div style={{ color: "#5f665f", display: "flex", fontSize: 30, marginTop: 20 }}>
